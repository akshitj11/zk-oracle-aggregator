...
impl ServerConfig {
    ...
    pub fn from_env() -> anyhow::Result<Self> {
        ...
        let sources_config = std::env::var("SOURCES_CONFIG")
            .map(PathBuf::from)
            .unwrap_or_else(|_| PathBuf::from("config/sources.integration.toml"));
        let proving_key_path = std::env::var("PROVING_KEY_PATH").ok().map(PathBuf::from);
...
pub fn load_source_pairs(path: &std::path::Path) -> anyhow::Result<Vec<(String, String)>> {
    let raw = std::fs::read_to_string(path)
        .with_context(|| format!("read sources config {}", path.display()))?;
    let file: SourcesFile = toml::from_str(&raw).context("parse sources config")?;
